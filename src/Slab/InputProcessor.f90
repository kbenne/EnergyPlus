...
SUBROUTINE ProcessInput(IDDFileName,IDFFileName)
  ...
  CHARACTER(len=*), INTENT(IN), OPTIONAL :: IDFFileName
   ...
   IF (NumObjectDefs == 0) THEN
     CALL ShowFatalError('No objects found in IDD.  Program will terminate.')
     ErrorsInIDD=.true.
   ENDIF
   ...
   IF (OverallErrorFlag) THEN
     ...
     CALL ShowFatalError('IP: Errors occurred on processing input file. Preceding condition(s) cause termination.')
   ENDIF
...
SUBROUTINE AddSectionDef(ProposedSection,ErrorsFound)
  ...
  IF (SqueezedSection /= Blank) THEN
    ...
    ENDIF
  ELSE
    CALL ShowSevereError('IP: Blank Sections not allowed.  Review audit.out file.',EchoInputFile)
    ErrFlag=.true.
    ErrorsFound=.true.
  ENDIF
