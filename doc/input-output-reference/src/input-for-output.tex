...
This field determines if any rounding will be performed for the values in the tabular reports when the Option field is set to TimeSeriesAndTabular. If No, all digits are shown after the decimal point without any rounding (23.238769213). If Yes, values are rounded for readability (23.24). The number of digits will vary depending on the type of value. This formatting is only applied to the tabular reports in the JSON file. This field has no effect on the time series data.
...
This field determines if any rounding will be performed for the values in the tabular reports when the Option field is set to SimpleAndTabular. If No, all digits are shown after the decimal point without any rounding (23.238769213). If Yes, values are rounded for readability (23.24). The number of digits will vary depending on the type of value. This formatting is only applied to the tabular reports in the SQL file. This field has no effect on the time series data and predefined database tables generated by the Simple option (see the list in Table \ref{table:tabular_unit_conversion_list}). This field is optional and, if not specified, defaults to Yes.
...
OutputControl:Files,
  ...
  No,                         !- ExtShd
  No,                         !- Tarcog
  No;                         !- Output Plant Component Sizing
\end{lstlisting}
