...
\subsection{Constructions - Inheriting Surface Constructions Instead of Hard-Assigning Them}\label{constructions---inheriting-surface-constructions-instead-of-hard-assigning-them}

Historically, every \hyperref[buildingsurfacedetailed]{BuildingSurface:Detailed}, \hyperref[fenestrationsurfacedetailed]{FenestrationSurface:Detailed}, and \hyperref[internalmass]{InternalMass} object had to be given an explicit \emph{Construction Name}. For large models this means repeating the same handful of construction names, by surface type and boundary condition, across hundreds or thousands of surfaces. Three objects -- \textbf{\hyperref[constructionassignmentset]{ConstructionAssignmentSet}}, \textbf{\hyperref[surfaceconstructionassignments]{SurfaceConstructionAssignments}}, and \textbf{\hyperref[subsurfaceconstructionassignments]{SubSurfaceConstructionAssignments}} -- let a construction be assigned once, at the \hyperref[building]{Building} or \hyperref[space]{Space} level, and inherited by every surface that leaves its own \emph{Construction Name} field blank.

The \emph{Construction Name} field on BuildingSurface:Detailed, FenestrationSurface:Detailed, and InternalMass is optional. When it is blank, EnergyPlus resolves the construction in the following order, stopping at the first one found (see Figure~\ref{fig:construction-assignment-inheritance}):

\begin{enumerate}
  \item
    An explicit (non-blank) \emph{Construction Name} on the surface itself.
  \item
    The \hyperref[constructionassignmentset]{ConstructionAssignmentSet} referenced by the surface's \hyperref[space]{Space} (\emph{Construction Assignment Set Name} field).
  \item
    The \hyperref[constructionassignmentset]{ConstructionAssignmentSet} referenced by the \hyperref[building]{Building} (\emph{Construction Assignment Set Name} field).
\end{enumerate}

If none of these resolves a construction, EnergyPlus issues a fatal error.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.45\textwidth, keepaspectratio=true]{media/construction-assignment-inheritance.png}
\caption{Construction resolution order: explicit assignment, then Space-level, then Building-level ConstructionAssignmentSet}
\label{fig:construction-assignment-inheritance}
\end{figure}

Within a ConstructionAssignmentSet, the slot used to resolve a surface depends on its type and outside boundary condition, as shown in Figure~\ref{fig:construction-assignment-set-structure} and Table~\ref{table:constructionassignmentset-slot-resolution} below. Sub-surface types resolve using their own declared Surface Type (ref: \hyperref[fenestrationsurfacedetailed]{FenestrationSurface:Detailed}), not the generic Window/Door class they are remapped to for heat transfer calculations.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth, keepaspectratio=true]{media/construction-assignment-set-structure.png}
\caption{Structure of ConstructionAssignmentSet, SurfaceConstructionAssignments, and SubSurfaceConstructionAssignments, and how their fields link to Construction}
\label{fig:construction-assignment-set-structure}
\end{figure}

In the table, ``window-type'' covers FixedWindow, OperableWindow, Window, Skylight, GlassDoor, TubularDaylightDome, and TubularDaylightDiffuser; ``door-type'' covers Door and OverheadDoor.

% table cas
\begin{longtable}[c]{@{}>{\raggedright}p{1.6in}>{\raggedright}p{1.7in}>{\raggedright\arraybackslash}p{2.6in}@{}}
    \caption{ConstructionAssignmentSet slot resolution \label{table:constructionassignmentset-slot-resolution}} \tabularnewline
    \toprule
    Surface/Sub-surface Type & Outside Boundary Condition & Slot \tabularnewline
    \midrule
    \endfirsthead

    \caption[]{ConstructionAssignmentSet slot resolution} \tabularnewline
    \toprule
    Surface/Sub-surface Type & Outside Boundary Condition & Slot \tabularnewline
    \midrule
    \endhead

    Wall / Floor / Roof or Ceiling & Outdoors & Exterior Surface Construction Assignments \tabularnewline
    Wall / Floor / Roof or Ceiling & Surface / Zone / Space & Interior Surface Construction Assignments \tabularnewline
    Wall / Floor / Roof or Ceiling & Ground / GroundFCfactorMethod / Foundation & Ground Contact Surface Construction Assignments \tabularnewline
    Wall / Floor / Roof or Ceiling & Adiabatic & Adiabatic Surface Construction Name (direct) \tabularnewline
    Window-type sub-surface & Exterior base surface & Exterior SubSurface Construction Assignments \tabularnewline
    Window-type sub-surface & Interior base surface & Interior SubSurface Construction Assignments \tabularnewline
    Door-type sub-surface & Exterior base surface & Exterior SubSurface Construction Assignments \tabularnewline
    Door-type sub-surface & Interior base surface & Interior SubSurface Construction Assignments \tabularnewline
    InternalMass & -- & Interior Partition Construction Name (direct) \tabularnewline
    \bottomrule
\end{longtable}

For an interzone pair (two surfaces facing each other across a Surface/Zone/Space boundary), when at least one side's construction is inherited rather than explicit, the side with the higher-precedence assignment (Explicit, then Space, then Building) governs the pair, and EnergyPlus finds or creates the reversed-layer-order construction for the other side -- reusing an existing construction with matching reversed layers if one is found, rather than creating a duplicate.

\subsection{ConstructionAssignmentSet}\label{constructionassignmentset}

This object defines a set of inherited construction assignments for surfaces and sub-surfaces, indexed by surface/sub-surface type and outside boundary condition. It can be referenced by a \hyperref[building]{Building} object or by one or more \hyperref[space]{Space} objects (Space-level references take precedence over the Building-level reference). Any \hyperref[buildingsurfacedetailed]{BuildingSurface:Detailed}, \hyperref[fenestrationsurfacedetailed]{FenestrationSurface:Detailed}, or \hyperref[internalmass]{InternalMass} object with a blank \emph{Construction Name} field resolves its construction through the referencing chain, in order: an explicit assignment on the surface itself, then the ConstructionAssignmentSet referenced by its Space, then the ConstructionAssignmentSet referenced by the Building.

\subsubsection{Inputs}\label{constructionassignmentset-inputs}

\paragraph{Field: Name}\label{constructionassignmentset-field-name}

This field is a user specified name that will be used as a reference by the \emph{Construction Assignment Set Name} field on \hyperref[building]{Building} and \hyperref[space]{Space} objects.

\paragraph{Field: Exterior Surface Construction Assignments Name}\label{constructionassignmentset-field-exterior-surface-construction-assignments-name}

The name of a \hyperref[surfaceconstructionassignments]{SurfaceConstructionAssignments} object giving the Floor, Wall, and Roof/Ceiling constructions for surfaces with Outside Boundary Condition = Outdoors.

\paragraph{Field: Interior Surface Construction Assignments Name}\label{constructionassignmentset-field-interior-surface-construction-assignments-name}

The name of a \hyperref[surfaceconstructionassignments]{SurfaceConstructionAssignments} object giving the Floor, Wall, and Roof/Ceiling constructions for surfaces with Outside Boundary Condition = Surface, Zone, or Space.

\paragraph{Field: Ground Contact Surface Construction Assignments Name}\label{constructionassignmentset-field-ground-contact-surface-construction-assignments-name}

The name of a \hyperref[surfaceconstructionassignments]{SurfaceConstructionAssignments} object giving the Floor, Wall, and Roof/Ceiling constructions for surfaces with Outside Boundary Condition = Ground, GroundFCfactorMethod, or Foundation.

\paragraph{Field: Exterior SubSurface Construction Assignments Name}\label{constructionassignmentset-field-exterior-subsurface-construction-assignments-name}

The name of a \hyperref[subsurfaceconstructionassignments]{SubSurfaceConstructionAssignments} object giving the constructions for sub-surfaces on exterior base surfaces.

\paragraph{Field: Interior SubSurface Construction Assignments Name}\label{constructionassignmentset-field-interior-subsurface-construction-assignments-name}

The name of a \hyperref[subsurfaceconstructionassignments]{SubSurfaceConstructionAssignments} object giving the constructions for sub-surfaces on interior base surfaces.

\paragraph{Field: Interior Partition Construction Name}\label{constructionassignmentset-field-interior-partition-construction-name}

The name of a \hyperref[construction-000]{Construction} used for \hyperref[internalmass]{InternalMass} objects with a blank \emph{Construction Name}.

\paragraph{Field: Adiabatic Surface Construction Name}\label{constructionassignmentset-field-adiabatic-surface-construction-name}

The name of a \hyperref[construction-000]{Construction} used for surfaces with Outside Boundary Condition = Adiabatic.
...
\subsection{SurfaceConstructionAssignments}\label{surfaceconstructionassignments}

This object defines the Floor, Wall, and Roof/Ceiling constructions for one outside-boundary-condition category (exterior, interior, or ground-contact), for reference by a \hyperref[constructionassignmentset]{ConstructionAssignmentSet}.

\subsubsection{Inputs}\label{surfaceconstructionassignments-inputs}

\paragraph{Field: Name}\label{surfaceconstructionassignments-field-name}

This field is a user specified name that will be used as a reference by \hyperref[constructionassignmentset]{ConstructionAssignmentSet} objects.

\paragraph{Field: Floor Construction Name}\label{surfaceconstructionassignments-field-floor-construction-name}

The name of a \hyperref[construction-000]{Construction} used for Floor surfaces in this category. May be left blank; a surface routed to this slot with this field blank falls through to the Building-level ConstructionAssignmentSet (if this SurfaceConstructionAssignments is referenced from a Space-level set).

\paragraph{Field: Wall Construction Name}\label{surfaceconstructionassignments-field-wall-construction-name}

The name of a \hyperref[construction-000]{Construction} used for Wall surfaces in this category. May be left blank, with the same fall-through behavior described above.

\paragraph{Field: Roof Ceiling Construction Name}\label{surfaceconstructionassignments-field-roof-ceiling-construction-name}

The name of a \hyperref[construction-000]{Construction} used for Roof/Ceiling surfaces in this category. May be left blank, with the same fall-through behavior described above.
...
\subsection{SubSurfaceConstructionAssignments}\label{subsurfaceconstructionassignments}

This object defines the constructions for each sub-surface type for one outside-boundary-condition category (exterior or interior), for reference by a \hyperref[constructionassignmentset]{ConstructionAssignmentSet}. The generic \emph{Window} Surface Type (ref: \hyperref[fenestrationsurfacedetailed]{FenestrationSurface:Detailed}) resolves via the Fixed Window Construction Name slot.

\subsubsection{Inputs}\label{subsurfaceconstructionassignments-inputs}

\paragraph{Field: Name}\label{subsurfaceconstructionassignments-field-name}

This field is a user specified name that will be used as a reference by \hyperref[constructionassignmentset]{ConstructionAssignmentSet} objects.

\paragraph{Field: Fixed Window Construction Name}\label{subsurfaceconstructionassignments-field-fixed-window-construction-name}

The name of a \hyperref[construction-000]{Construction} used for FixedWindow and Window sub-surfaces in this category. May be left blank, in which case a sub-surface routed to this slot falls through to the Building-level ConstructionAssignmentSet (if this SubSurfaceConstructionAssignments is referenced from a Space-level set).

\paragraph{Field: Operable Window Construction Name}\label{subsurfaceconstructionassignments-field-operable-window-construction-name}

The name of a \hyperref[construction-000]{Construction} used for OperableWindow sub-surfaces in this category. May be left blank, with the same fall-through behavior described above.

\paragraph{Field: Door Construction Name}\label{subsurfaceconstructionassignments-field-door-construction-name}

The name of a \hyperref[construction-000]{Construction} used for Door sub-surfaces in this category. May be left blank, with the same fall-through behavior described above.

\paragraph{Field: Glass Door Construction Name}\label{subsurfaceconstructionassignments-field-glass-door-construction-name}

The name of a \hyperref[construction-000]{Construction} used for GlassDoor sub-surfaces in this category. May be left blank, with the same fall-through behavior described above.

\paragraph{Field: Overhead Door Construction Name}\label{subsurfaceconstructionassignments-field-overhead-door-construction-name}

The name of a \hyperref[construction-000]{Construction} used for OverheadDoor sub-surfaces in this category. May be left blank, with the same fall-through behavior described above.

\paragraph{Field: Skylight Construction Name}\label{subsurfaceconstructionassignments-field-skylight-construction-name}

The name of a \hyperref[construction-000]{Construction} used for Skylight sub-surfaces in this category. May be left blank, with the same fall-through behavior described above.

\paragraph{Field: Tubular Daylight Dome Construction Name}\label{subsurfaceconstructionassignments-field-tubular-daylight-dome-construction-name}

The name of a \hyperref[construction-000]{Construction} used for TubularDaylightDome sub-surfaces in this category. May be left blank, with the same fall-through behavior described above.

\paragraph{Field: Tubular Daylight Diffuser Construction Name}\label{subsurfaceconstructionassignments-field-tubular-daylight-diffuser-construction-name}

The name of a \hyperref[construction-000]{Construction} used for TubularDaylightDiffuser sub-surfaces in this category. May be left blank, with the same fall-through behavior described above.
...
The pressure (Pa) which will be considered to be the limit for vacuum glazing pressure. All pressures less than or equal to this pressure will be considered to be vacuum. Default: 13.238 Pa. It is used only when WindowThermalModel:Params DeflectionModel = TemperatureAndPressureInput.

\paragraph{Field: Initial Temperature}\label{field-initial-temperature}

The temperature (\(^{o}\)C) of the gap in the time of fabrication. Default: 25 \(^{o}\)C. It is used only when WindowThermalModel:Params DeflectionModel = TemperatureAndPressureInput.

\paragraph{Field: Initial Pressure}\label{field-initial-pressure}

The pressure (Pa) of the gap at the time of fabrication of the sealed glazing system unit. Default: 101325 Pa. It is used only when WindowThermalModel:Params DeflectionModel = TemperatureAndPressureInput.
